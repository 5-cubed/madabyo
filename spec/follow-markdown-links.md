...
| Given `activeFileId` `C:\ws\main.md` and `workspacePath` `C:\ws` - When the user clicks a relative link `other.md` - Then a new tab for `other.md` opens and becomes active (not blocked) | Windows sibling link | `web/src/App.link-follow.test.jsx` |
| Given a Windows-style workspace - When a clicked link resolves outside `workspacePath` - Then the tab opens with `status: 'blocked'`, same as POSIX today | Windows workspace guard | `web/src/App.link-follow.test.jsx` |
| Given a Windows-style resolved link - When the file opens - Then the path passed to `paneManager.openFile` is `C:/ws/other.md`, not `/C:/ws/other.md` | Windows path normalization | `web/src/App.link-follow.test.jsx` |
| Given all 7 existing POSIX link-follow cases - When this Windows fix lands - Then all 7 cases still pass unchanged | Regression: POSIX intact | `web/src/App.link-follow.test.jsx` |
